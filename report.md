...
图像还原实验优先读取 `img/` 目录下最多 3 张图片作为清晰目标图。如果 `img/` 为空，脚本会使用 CamVid 真实图片作为还原源图。
...
Restoration U-Net 相比退化输入明显降低 MSE，并将 PSNR 从 29.9817 dB 提升到 35.7171 dB，SSIM 从 0.7965 提升到 0.9277。Plain CNN 也有提升，但三个指标均低于 Restoration U-Net。
